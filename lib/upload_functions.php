...
if ( isset($_POST["submit"]) AND $_POST["submit"] == "Opladen" ) //als er een form gesubmit werd
{
    foreach ( $_FILES as $f )   //overloop alle bestanden in $_FILES
    {
        $upfile = array();
        ...
        else
        {
            ...
            if ( move_uploaded_file( $upfile["tmp_name"], $upfile["target_path_name"]))  echo "The file " . $upfile["name"] . " has been uploaded.<br>";
            else echo "Sorry, there was an unexpected error uploading file " . $upfile["name"] . "<br>";
        }
    }
}
...
function CheckUploadedFile( $upfile, $check_real_image = true, $check_if_exists = true, $check_max_size = true, $check_allowed_extensions = true )
{
    ...
    if ( $check_real_image AND $upfile["getimagesize"] === false )          { echo "File " . $upfile["name"] . " is not an image.<br>"; $returnvalue = false; }
    if ( $check_if_exists AND file_exists($upfile["target_path_name"]))         { echo "File  " . $upfile["name"] . " already exists.<br>"; $returnvalue = false; }  // Check if file already exists
    if ( $check_max_size AND $upfile["size"] > 500000 )                               { echo "File  " . $upfile["name"] . "  is too large.<br>"; $returnvalue = false;  }  // Check file size

    if ( $check_allowed_extensions )  // Allow only certain file formats
    {
